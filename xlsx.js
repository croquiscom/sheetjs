...
function parse_borders(t, styles, themes, opts) {
	...
	t[0].match(tagregex).forEach(function(x) {
		...
		switch (y[0]) {
			...
			/* 18.8.? color CT_Color */
			case '<color':
				if (!sub_border.color) sub_border.color = {};
				if (y.rgb) sub_border.color.rgb = y.rgb.slice('-6');
			case '<color>': break;
...
function safe_format(p, cf, opts, themes, styles) {
	...
	// cell.s에 fill에 대한 정보만 포함되기 때문에 cell.s.fill로 분리한다.
	p.s = {};
	if(cf && cf.fillId != null) try {
		var fill = styles.Fills[cf.fillId];
		if (fill.fgColor && fill.fgColor.theme && !fill.fgColor.rgb) {
			fill.fgColor.rgb = rgb_tint(themes.themeElements.clrScheme[fill.fgColor.theme].rgb, fill.fgColor.tint || 0);
			if(opts.WTF) fill.fgColor.raw_rgb = themes.themeElements.clrScheme[fill.fgColor.theme].rgb;
		}
		if (fill.bgColor && fill.bgColor.theme) {
			fill.bgColor.rgb = rgb_tint(themes.themeElements.clrScheme[fill.bgColor.theme].rgb, fill.bgColor.tint || 0);
			if(opts.WTF) fill.bgColor.raw_rgb = themes.themeElements.clrScheme[fill.bgColor.theme].rgb;
		}
		p.s.fill = fill;
	} catch(e) { if(opts.WTF && styles.Fills) throw e; }

	// font, border, alignment 정보도 함께 cell.s에 저장한다.
	if(cf && cf.fontId != null) try {
		p.s.font = styles.Fonts[cf.fontId];
	} catch(e) { if(opts.WTF && styles.Fonts) throw e; }
	if(cf && cf.borderId != null) try {
		p.s.border = styles.Borders[cf.borderId];
	} catch(e) { if(opts.WTF && styles.Borders) throw e; }
	if(cf && cf.alignment != null) {
		p.s.alignment = cf.alignment;
	}
}
...
function write_ws_xml_cell(cell, ref, ws, opts, idx, wb) {
	...
	var v = vv ? writetag('v', escapexml(vv)) : '';
	var o = ({r:ref});
	/* TODO: cell style */
...
return function parse_ws_xml_data(sdata, s, opts, guess, themes, styles) {
	...
	for(var marr = sdata.split(rowregex), mt = 0, marrlen = marr.length; mt != marrlen; ++mt) {
		...
		for(ri = 0; ri != cells.length; ++ri) {
			...
			/* formatting */
			cf = (do_format && tag.s !== undefined) ? cf = styles.CellXf[tag.s] : null;
			fmtid = (cf != null && cf.numFmtId != null) ? cf.numFmtId : 0;
...
// from: https://github.com/protobi/js-xlsx
/////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		_addFont: function (attributes) {
			...
			if (attributes.color) {
				if (attributes.color.theme) {
					...
					attributes.color.rgb = attributes.color.rgb.slice(-6);

					if (attributes.color.rgb.length == 6) {
						attributes.color.rgb = "FF" + attributes.color.rgb /// add alpha to an RGB as Excel expects aRGB
					}
		...
		_addFill: function (attributes) {
			...
			if (attributes.fgColor) {
				...
				if (attributes.fgColor.rgb) {
					attributes.fgColor.rgb = attributes.fgColor.rgb.slice(-6);
			...
			if (spec) {
				...
				if (spec.color) {
					...
					if (spec.color.auto) {
						...
						spec.color.rgb = spec.color.rgb.slice(-6);

						if (spec.color.rgb.length == 6) {
							spec.color.rgb = "FF" + spec.color.rgb
						}
